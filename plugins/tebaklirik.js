...
`.trim()
    conn.tebaklirik[id] = [
        await conn.send2Button(m.chat, caption.trim(), 'Made By Johannes ♥️', 'Bantuan', `${usedPrefix}teli`, 'Give up', 'nyerah'),
        json, poin,
        setTimeout(async () => {
            if (conn.tebaklirik[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.jawaban}*`, '', 'Lagi bang', `#tebaklirik`)
