...
let handler = async (m, { conn, command }) => {
  if (/^tod$/i.test(command)) {
    conn.send2Button(m.chat, 'Truth or Dare', 'by:johannes | pesan otomatis', 'TRUTH', '#truth', 'DARE', '#dare', 'RANDOM', `${pickRandom([',dare', ',truth'])}`)
  }
  if (/^truth$/i.test(command)) {
    ...
    conn.send2Button(m.chat, json.message, '', 'TRUTH', '#truth', 'DARE', '#dare', 'RANDOM', `${pickRandom([',dare', ',truth'])}`)
